...
mkdir -p "$DEST_DIR"/{font2d,Image,md2,obj,Scene,Shaders/gles,Shaders/opengl,Sound/wav,Songs}

# 3. Copy Assets (Simplified with wildcards)
cp "$SRC_DIR"/*.txt "$DEST_DIR/" 
cp -R "$SRC_DIR"/{font2d,Gui,Songs,md2} "$DEST_DIR/"
...
find . -type f \( -name "*.txt" -o -name "*.csv" -o -name "*.png" -o -name "*.obj" -o -name "*.mtl" -o -path "./md2/*.md2" \) | while read -r f; do
