...
void ScoreBuilder::Write(GuiMusicScore& gui)
{
  ...
  for (const Note& n : m_notes)
  {
    ...
    Vec2f pos(x, y);
    char ch = 'w'; // TODO 
    GuiMusicScore::Glyph g(ch, pos, scale);
    gui.AddGlyph(g);

    // Add ledger lines
    for (int i = 5; i < n.m_stavePos; i += 2)
    {
      gui.AddGlyph(GuiMusicScore::Glyph('_', Vec2f(x, static_cast<float>(i + 5) * HEIGHT_SCALE), scale));
    }

    for (int i = -6; i >= n.m_stavePos; i -= 2)
    {
      gui.AddGlyph(GuiMusicScore::Glyph('_', Vec2f(x, static_cast<float>(i + 4)* HEIGHT_SCALE), scale));
    }
