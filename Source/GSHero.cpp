...
std::cout << "*** Pause resume time: " << m_pauseResumeTime << "\n";
  ...
  //  first, find the next beat after m_pauseResumeTime
...
std::cout << "VERY STRANGE, on resuming, we seem to be at the end of the song?\n";
  ...
  while (!goodTimeFound)
  {
    GoToFirstBeatOfBar(beats, it, m_pauseResumeTime);
   
    if (NoteEventWasFoundAtTime(m_pauseResumeTime, noteEvents))
    {
    ...
    else if (it == beats.begin())
    {
      ...
      goodTimeFound = true;
      m_pauseResumeTime = 0; // we might as well restart
    }
    else
    {
