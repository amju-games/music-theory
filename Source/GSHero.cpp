...
void GSHero::FindResumePoint()
{
  // Find the best place to resume, given the current time, which is when
  //  the game round was paused.
  m_pauseResumeTime = Resumer::FindResumePoint(
    m_pauseResumeTime, 
...
{
  StartCountInSongAndGui();

  // (We start the main song at the resume time, when the time for
  //   the count in has expired. See Update().)
}

void GSHero::StartCountInSongAndGui()
{
